...
int parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
	...
	if ( pscnt > 0 ) {
		...
		switch( op ) {
			...
			case ADD: case SUB: case SLT: case SLTU: case AND: case OR: case XOR: case SLL: case SRL: case SRA:  case MIN: case MINU: case ORN: //case CLMUL: case CLMULH: case CLMULR:
			case ANDN: case MAX: case MAXU: case ROL: case ROR: case SH1ADD: case SH2ADD: case SH3ADD: case XNOR: 
				if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line, "Invalid format" );
				i->a1.reg = parse_reg(o1, line);
				i->a2.reg = parse_reg(o2, line);
				i->a3.reg = parse_reg(o3, line);
			...
			case CLZ: case ZEXTH:
				...
				i->a2.reg = parse_reg(o2, line);
				return 1;
			case BCLRI: case BSETI: case RORI:
				if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line, "Invalid format" );
				i->a1.reg = parse_reg(o1, line);
				i->a2.reg = parse_reg(o2, line);
				i->a3.imm = parse_imm(o3, 5, line);
...
void execute(uint8_t* mem, instr* imem, label_loc* labels, int label_count, bool start_immediate) {
	...
	while(!dexit) {
		...
		switch (i.op) {
			// micah 4
			case CPOP:
				// NOTE: + has precedence over binary &
				rf[i.a1.reg] = (rf[i.a2.reg] & 0x55555555) + ((rf[i.a2.reg]>>1) & 0x55555555); 
				rf[i.a1.reg] = (rf[i.a1.reg] & 0x33333333) + ((rf[i.a1.reg]>>2) & 0x33333333);
				rf[i.a1.reg] = (rf[i.a1.reg] & 0x0f0f0f0f) + ((rf[i.a1.reg]>>4) & 0x0f0f0f0f);
				rf[i.a1.reg] = (rf[i.a1.reg] & 0x00ff00ff) + ((rf[i.a1.reg]>>8) & 0x00ff00ff);
				rf[i.a1.reg] = (rf[i.a1.reg] & 0x0000ffff) + ((rf[i.a1.reg]>>16) & 0x0000ffff);
				break;

			case CTZ:
				uint32_t count, shift, x;
				x = rf[i.a2.reg];
				count = ((x & 0xFFFF) == 0) << 4; // AND 00000000000000001111111111111111 if 0 then right half 16 bits zeros
				x >>=count; // leave the lower half if there's 1 in there; otherwise leave the higher half
				shift = ((x & 0xFF) == 0) << 3; // AND 0000000011111111, if 0 then right 8 bits zeros
				x >>=shift;
				count |= shift; // in this case or is the same as add
				shift = ((x & 0xF) == 0) << 2; // AND 00001111
				x >>=shift;
				count |= shift;
				shift = ((x & 0x3) == 0) << 1; // AND 0011
				x >>=shift;
				count |= shift;
				shift = ((x & 0x1) == 0); // AND 01
				x >>=shift;
				count |= shift;
				rf[i.a1.reg] = count;
				break;
			...
			case RORI:
				i.a3.imm &= 0x1f; // only the right 5 bits are used in shamt
				rf[i.a1.reg] = (rf[i.a2.reg] >> i.a3.imm) | (rf[i.a2.reg] << (32 - i.a3.imm));
				break;

			case SH1ADD:
				rf[i.a1.reg] =  (rf[i.a2.reg] << 1) + rf[i.a3.reg]; break;

			case SH2ADD:
				rf[i.a1.reg] =  (rf[i.a2.reg] << 2) + rf[i.a3.reg]; break;

			case SH3ADD:
				rf[i.a1.reg] =  (rf[i.a2.reg] << 3) + rf[i.a3.reg]; break;

			case XNOR:
				rf[i.a1.reg] = ~(rf[i.a2.reg] ^ rf[i.a3.reg]); break;

			case ZEXTH:
				rf[i.a1.reg] = rf[i.a2.reg] & 0xFFFF; break;
			...
			case BEXT: 
				rf[i.a1.reg] = rf[i.a3.reg] & (32 - 1);
				rf[i.a1.reg]  = (rf[i.a2.reg] >> rf[i.a1.reg]) & 1;
				break;
			case BEXTI: 
				rf[i.a1.reg]  = i.a3.imm & (32 - 1);
				rf[i.a1.reg] = (rf[i.a2.reg] >> rf[i.a1.reg]) & 1;
				break;
			case BINV: 
				rf[i.a1.reg] = rf[i.a3.reg] & (32 - 1);
			...
			case BSET: 
				...
				break;
			case SEXTB: 
				index =  (int32_t)(rf[i.a2.reg]  << 24);
				rf[i.a1.reg] =  (uint32_t)(index >> 24); 
				break;
			case SEXTH: 
				index =  (int32_t)(rf[i.a2.reg]  << 16);
				rf[i.a1.reg] =  (uint32_t)(index >> 16); 
				break;
			...
			// 		rf[i.a1.reg] = (((int64_t)rf[i.a3.reg]>>index) & 1) ? ((int64_t)rf[i.a1.reg] ^ ((int64_t)rf[i.a2.reg] << index)):rf[i.a1.reg];
			// 	break;
			// case CLMULH:
			// 	rf[i.a1.reg] = 0;
			// 	for(int index = 1; index <=32; ++index)
			// 		rf[i.a1.reg] = (((int64_t)rf[i.a3.reg]>>index) & 1) ? ((int64_t)rf[i.a1.reg] ^ ( (int64_t)rf[i.a2.reg] >> (32-index) )):(int64_t)rf[i.a1.reg];
			// 	break;
			// case CLMULR:
			// 	rf[i.a1.reg] = 0;
			// 	for(int index=0; index<=31; ++index)
			// 		rf[i.a1.reg] = (((int64_t)rf[i.a3.reg] >> index) & 1) ? ((int64_t)rf[i.a1.reg] ^ ((int64_t)rf[i.a2.reg] >> (32-index-1))):(int64_t)rf[i.a1.reg];
			// 	break;
			case MIN:
				rf[i.a1.reg] = ((int32_t)rf[i.a2.reg] < (int32_t)rf[i.a3.reg]) ? (int32_t)rf[i.a2.reg]:(int32_t)rf[i.a3.reg];
				break;
			case MINU:
			...
			case ORCB:
				rf[i.a1.reg] = 0;
				for (int index=8; index<=32; index = index + 8)
					rf[i.a1.reg] |= ( ( ( rf[i.a2.reg] << (32 - index) ) >>24 ) == 0) ? 0:(0b11111111 << (index-8));
				break;
			case ORN: rf[i.a1.reg] = rf[i.a2.reg] | ~rf[i.a3.reg]; break;
			case REV8:
				rf[i.a1.reg] = 0;
				for (int index=0; index<=24; index = index+8)
					rf[i.a1.reg] |= ((rf[i.a2.reg] << index)>>24) << index;
				break;
			...
			case MAX: rf[i.a1.reg] = (*(int32_t*)&rf[i.a2.reg]) < (*(int32_t*)&rf[i.a3.reg]) ? (*(int32_t*)&rf[i.a3.reg]) : (*(int32_t*)&rf[i.a2.reg]); break;
			case MAXU: rf[i.a1.reg] = rf[i.a2.reg] < rf[i.a3.reg] ? rf[i.a3.reg] : rf[i.a2.reg]; break;
			case ROL: 
				{
					uint32_t mask = 0x0000001f ;
					int shamt = rf[i.a3.reg] & mask;
					rf[i.a1.reg] = (rf[i.a2.reg] << shamt) | (rf[i.a2.reg] >> (31 - shamt) >> 1);
					break;
				}
			case ROR:
				{
			...
			case BSETI: rf[i.a1.reg] = rf[i.a2.reg]|(1 << i.a3.imm); break;			
